...
@mcp.tool()
async def connect(connection_string: str | None = None) -> dict:
    ...
    try:
        ...
        return {"result": "Connected to MongoDB successfully."}
    except ConnectionError as e:
        logger.error(f"Connect failed: {e}", exc_info=True)
        return {"result": f"Failed to connect: {e}"}
...
async def disconnect() -> dict:
    ...
    return {"result": "Disconnected from MongoDB."}
